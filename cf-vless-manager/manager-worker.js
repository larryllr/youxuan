...
<section><h3>套餐和分组</h3><div class=row><input id=pname placeholder="套餐名称"><input id=pquota placeholder="套餐总流量 GB，0 不限"><input id=pdays placeholder="有效天数，0 永不过期"><select id=preset><option value=none>不自动重置</option><option value=monthly>月租：每月重置流量</option></select><input id=presetday placeholder="每月几号重置，默认 1"><button onclick=createPlan()>创建套餐</button><input id=gname placeholder="分组名称"><button onclick=createGroup()>创建分组</button></div><div id=plans></div><div id=groups></div></section>
