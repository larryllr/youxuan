...
async function usersApi(request, env, id, action) {
  ...
  if (request.method === 'PATCH') {
    ...
    if ('used_upload_gb' in b) b.used_upload = gbToBytes(b.used_upload_gb);
    if ('used_download_gb' in b) b.used_download = gbToBytes(b.used_download_gb);
    if ('valid_days' in b) b.expires_at = daysToExpiry(b.valid_days, unix());
    const allowed = ['email', 'name', 'uuid', 'sub_token', 'plan_id', 'quota_bytes', 'used_upload', 'used_download', 'expires_at', 'next_reset_at', 'device_limit', 'speed_limit_bps', 'status', 'note'];
...
async function tableApi(request, env, table, id, allowed, defaults) {
  ...
  if (request.method === 'DELETE') {
    ...
    if (table === 'nodes') {
      ...
      await env.DB.batch([
        env.DB.prepare("UPDATE users SET plan_id=NULL,next_reset_at=0,updated_at=? WHERE plan_id=?").bind(now, id),
...
<section><h3>分组管理</h3><div class=row><input id=gname placeholder="分组名称"><button onclick=createGroup()>创建分组</button></div><div id=groups></div></section>
<section><h3>最近连接</h3><div id=sessions></div></section></div>
<script>
const $=id=>document.getElementById(id),DH=${JSON.stringify(defaultProxyHost)};let userRows=[],nodeRows=[],planRows=[],limitRows=[],editingUser='',editingNode='',editingPlan='';const L={users:'用户总数',activeUsers:'正常用户',activeNodes:'启用节点',activeSessions:'在线连接',trafficBytes:'已用流量',email:'账号',name:'名称',status:'状态',plan_name:'套餐',groups:'分组',used:'已用/总量',expire:'到期时间',next_reset:'下次重置',sub:'订阅链接 / 操作',id:'ID',address:'入口地址',port:'端口',host:'Host',path:'路径',group_id:'分组',enabled:'启用',node_ops:'节点操作',plan_ops:'套餐操作',default_user_quota_bytes:'节点默认上限',quota_bytes:'总流量',valid_days:'有效天数',device_limit:'设备数',speed_limit_bps:'限速',price:'价格',sort_order:'排序',reset_policy:'月租重置',node_name:'节点',quota_limit:'节点上限',node_used:'节点已用',upload:'上传',download:'下载',reason:'断开原因'};
...
async function loadAll(){try{msg.textContent='正在加载...';let s=await api('summary');sum.innerHTML=Object.entries(s.data).map(([k,v])=>'<div class=card><b>'+h(L[k]||k)+'</b><br>'+(/Bytes/.test(k)?b(v):h(v))+'</div>').join('');let [us,ns,ps,gs,ss]=await Promise.all([api('users'),api('nodes'),api('plans'),api('groups'),api('sessions')]);userRows=us.data||[];nodeRows=ns.data||[];planRows=ps.data||[];uplan.innerHTML='<option value="">不绑定套餐</option>'+planRows.filter(p=>p.enabled).map(p=>'<option value="'+h(p.id)+'">'+h(p.name)+'</option>').join('');fillLimitUsers();users.innerHTML=tbl(userRows,['email','name','status','plan_name','groups','used','expire','next_reset','sub']);nodes.innerHTML=tbl(nodeRows,['id','name','address','port','host','path','group_id','default_user_quota_bytes','enabled','node_ops']);plans.innerHTML=tbl(planRows,['id','name','quota_bytes','valid_days','reset_policy','device_limit','speed_limit_bps','price','sort_order','enabled','plan_ops']);groups.innerHTML=tbl(gs.data,['id','name']);sessions.innerHTML=tbl(ss.data,['id','email','node_name','status','upload','download','reason']);msg.textContent='数据已刷新';}catch(e){msg.textContent='错误：'+e.message}}
...
async function deleteUser(id){if(!confirm('确定删除这个用户吗？该用户的订阅、节点限额、连接和用量记录都会删除。'))return;await api('users/'+id,{method:'DELETE'});msg.textContent='用户已删除';loadAll()}
function userBody(){let id=safeId(uid.value||uemail.value||uname.value||('user-'+Date.now()));uid.value=id;let body={id,email:uemail.value,name:uname.value,uuid:uuid.value.trim()||undefined,sub_token:usub.value.trim()||undefined,plan_id:uplan.value||null,group_ids:ugroups.value.split(',').map(x=>x.trim()).filter(Boolean),quota_gb:uq.value===''?undefined:uq.value,device_limit:Number(udev.value||0),speed_limit_bps:Number(uspeed.value||0),status:ustatus.value,note:unote.value};if(uup.value!=='')body.used_upload_gb=uup.value;if(udown.value!=='')body.used_download_gb=udown.value;if(uexp.value!=='')body.expires_at=Number(uexp.value||0);else if(udays.value!=='')body.valid_days=udays.value;if(unext.value!=='')body.next_reset_at=Number(unext.value||0);return body}
...
async function editUser(id){let res=await api('users/'+id),r=res.data;if(!r)return;uid.value=r.id;uid.readOnly=true;uemail.value=r.email||'';uname.value=r.name||'';uuid.value=r.uuid||'';usub.value=r.sub_token||'';uplan.value=r.plan_id||'';ugroups.value=(r.group_ids||[]).join(',');uq.value=gbv(r.quota_bytes);uup.value=gbv(r.used_upload);udown.value=gbv(r.used_download);udays.value='';uexp.value=r.expires_at||0;unext.value=r.next_reset_at||0;udev.value=r.device_limit||0;uspeed.value=r.speed_limit_bps||0;ustatus.value=r.status||'active';unote.value=r.note||'';editingUser=id;usave.textContent='保存修改';msg.textContent='正在编辑用户：'+id;uid.scrollIntoView({behavior:'smooth',block:'center'})}
function resetUserForm(){editingUser='';uid.readOnly=false;for(const x of [uid,uemail,uname,uuid,usub,ugroups,uq,uup,udown,udays,uexp,unext,udev,uspeed,unote])x.value='';uplan.value='';ustatus.value='active';usave.textContent='保存用户'}
...
function editPlan(id){let r=planRows.find(x=>x.id===id);if(!r)return;pid.value=r.id;pname.value=r.name||'';pquota.value=gbv(r.quota_bytes);pdays.value=r.valid_days||0;pdev.value=r.device_limit||0;pspeed.value=r.speed_limit_bps||0;pprice.value=(Number(r.price_cents||0)/100).toFixed(2);pcurrency.value=r.currency||'CNY';preset.value=r.reset_cycle||'none';presetday.value=r.reset_day||1;psort.value=r.sort_order||0;penabled.value=r.enabled?1:0;editingPlan=id;psave.textContent='保存修改';msg.textContent='正在编辑套餐：'+id;pid.scrollIntoView({behavior:'smooth',block:'center'})}
function resetPlanForm(){editingPlan='';for(const x of [pid,pname,pquota,pdays,pdev,pspeed,pprice,pcurrency,presetday,psort])x.value='';preset.value='none';penabled.value=1;psave.textContent='保存套餐'}
async function togglePlan(id,en){await api('plans/'+id,{method:'PATCH',body:JSON.stringify({enabled:en})});msg.textContent=en?'套餐已启用':'套餐已禁用';loadAll()}
async function deletePlan(id){if(!confirm('确定删除套餐 '+id+' 吗？已绑定用户会变成不绑定套餐，但保留当前流量和到期设置。'))return;await api('plans/'+id,{method:'DELETE'});if(editingPlan===id)resetPlanForm();msg.textContent='套餐已删除';loadAll()}
