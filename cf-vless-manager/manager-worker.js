...
function saveSubRegion(){let r=currentSubRegion();localStorage.subRegion=r;subRegion.value=['','US','HK','JP','SG','KR','DE','NL','GB'].includes(r)?r:'';if(subRegion.value)subRegionCustom.value='';regionState.textContent=r?('临时覆盖为 '+r):'按节点代码地区';if(isLight())renderLight();else if(userRows.length)users.innerHTML=tbl(userRows,['email','name','status','groups','expire','hours','sub']);msg.textContent=r?'临时订阅地区：'+r:'已恢复按节点代码地区'}
...
function lightUrl(){let ips=lightList().join(','),r=currentSubRegion(),q=[];if(r)q.push('region='+encodeURIComponent(r));if(ips)q.push('ips='+encodeURIComponent(ips));return lightBase()+(q.length?'?'+q.join('&'):'')}
async function loadLightSettings(){try{let r=await api('light-settings/'+lightProject());let d=r.data||{};lightIps.value=d.ips||localStorage.getItem(lightKey())||'';let reg=cleanRegion(d.region||'');if(reg){subRegion.value=['US','HK','JP','SG','KR','DE','NL','GB'].includes(reg)?reg:'';subRegionCustom.value=subRegion.value?'':reg;regionState.textContent='临时覆盖为 '+reg}else if(!localStorage.subRegion){subRegion.value='';subRegionCustom.value='';regionState.textContent='按节点代码地区'}renderLight();await loadLightNodeSort()}catch(e){lightIps.value=localStorage.getItem(lightKey())||'';renderLight();await loadLightNodeSort().catch(()=>{});msg.textContent='读取保存节点失败：'+e.message}}
function renderLight(){lightOut.textContent='订阅地址:\\n'+lightUrl()+'\\n\\n当前只生成 /llr/sub 链接，不改变 '+(CUR?.host||DH)+' 的连接逻辑；自定义节点会保存到管理 Worker。'}
function extractLightIps(){let v=lightIps.value.trim();try{let u=new URL(v);let ips=u.searchParams.get('ips')||'';let r=cleanRegion(u.searchParams.get('region')||'');if(r){subRegion.value=['US','HK','JP','SG','KR','DE','NL','GB'].includes(r)?r:'';subRegionCustom.value=subRegion.value?'':r;saveSubRegion()}if(ips)lightIps.value=ips.split(',').map(x=>x.trim()).filter(Boolean).join('\\n')}catch(e){}renderLight();msg.textContent='已按一行一个整理'}
async function saveLightIps(){localStorage.setItem(lightKey(),lightIps.value);await api('light-settings/'+lightProject(),{method:'PATCH',body:JSON.stringify({ips:lightIps.value,region:currentSubRegion()})});renderLight();msg.textContent='自定义节点已保存到管理 Worker'}
async function clearLightIps(){localStorage.removeItem(lightKey());lightIps.value='';await api('light-settings/'+lightProject(),{method:'PATCH',body:JSON.stringify({ips:'',region:currentSubRegion()})});renderLight();msg.textContent='已清空并保存'}
...
async function copySub(tok){let u=location.origin+'/sub/'+tok+regionQuery();await navigator.clipboard.writeText(u);msg.textContent='已复制 '+DH+' 项目的订阅链接：'+u}
